...
int main(int argc, char** argv) {
	...
	ComPtr<ID3D12DescriptorHeap> srv_desc_heap = nullptr;
	...
	ImGui_ImplDX12_Init(device.Get(), FRAME_COUNT,
		DXGI_FORMAT_R8G8B8A8_UNORM_SRGB, srv_desc_heap.Get(),
	...
	while (!done) {
		...
		command_list->SetDescriptorHeaps(1, srv_desc_heap.GetAddressOf());
		...
		if (time >= 1.0) {
			...
			snprintf(title, sizeof(title), "TINY Window Manager - Direct3D 12 With ImGUI - FPS: %d / Frame Time: %.2f ms", fps, frame_time * 1000);
