...
int main(int argc, char** argv) {
	...
	twm_window window = twm_create_window("TINY Window Manager - Direct3D 11 With ImGUI", TWM_CENTER, TWM_CENTER, width, height, TWM_WINDOW_DEFAULT);
	...
	while (!done) {
		...
		if (time >= 1.0) {
			...
			snprintf(title, sizeof(title), "TINY Window Manager - Direct3D 11 With ImGUI - FPS: %d / Frame Time: %.2f ms", fps, frame_time * 1000);
